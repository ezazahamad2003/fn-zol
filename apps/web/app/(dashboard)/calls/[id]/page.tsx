  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          <Link href="/dashboard" className="text-xs text-muted-foreground hover:underline">← All calls</Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          ...
          <CardContent className="p-3"><Transcript value={c.transcript} /></CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle>Run trace</CardTitle></CardHeader>
          <CardContent className="p-3"><RunTrace toolCalls={(toolCalls ?? []) as ToolCall[]} /></CardContent>
        </Card>
      </div>
          ...
          <CardContent className="p-3 text-sm space-y-2">
            ...
            {(appointments as Appointment[] | null)?.map((a) => (
              <div key={a.id} className="border border-border rounded-md p-2">
                ...
                <div className="text-xs text-muted-foreground">{formatDateTime(a.start_at)} → {formatDateTime(a.end_at)}</div>
        ...
        <Card>
          ...
          <CardContent className="p-3 text-sm space-y-2">
            ...
            {(messages as Message[] | null)?.map((m) => (
              <div key={m.id} className="border border-border rounded-md p-2">
                <div className="flex items-center justify-between gap-2">
                  <span className="font-medium">{m.customer_name ?? "(anon)"}</span>
